import type { ComposeDslContext, ComposeNode } from "compose-dsl";
...
function getLang(): "zh" | "en" {
  try {
    const opCtx = (globalThis as any).OperitContext;
...
function buildStatusModel(result: Record<string, unknown>): StatusModel {
  return {
...
/* ─── UI ─── */
function buildScreen(ctx: ComposeDslContext): ComposeNode {
  const t = resolveText();
  const UI = ctx.UI;
  const M = ctx.Modifier;
  const stateRef = ctx.useRef<SettingsState>("feishuSettingsState", INITIAL_STATE);
  const tickRef = ctx.useRef<number>("feishuSettingsTickRef", 0);
  const busyRef = ctx.useRef<boolean>("feishuSettingsBusy", false);
  const loadedRef = ctx.useRef<boolean>("feishuSettingsLoaded", false);
  const [, forceRender] = ctx.useState<number>("feishuSettingsTick", 0);

  function patchState(patch: Partial<SettingsState>): void {
  ...
  async function importModelsFromOperit(): Promise<void> {
    ...
    try {
      ...
      const anyGlobal = globalThis as unknown as {
        Tools?: {
          SoftwareSettings?: {
            listModelConfigs?: () => Promise<Record<string, unknown>>;
          };
        };
      };
      const listFn = anyGlobal.Tools?.SoftwareSettings?.listModelConfigs;
      if (typeof listFn === "function") {
        res = await callFnWithTimeout(
          () => listFn.call(anyGlobal.Tools?.SoftwareSettings) as Promise<Record<string, unknown>>,
          15000
        );
      } else {
        res = (await callToolWithTimeout(
          "list_model_configs",
          {},
          15000
        )) as Record<string, unknown>;
      ...
      const configs = Array.isArray(res && res.configs)
        ? (res.configs as Array<Record<string, unknown>>)
        : [];
      const options: ModelOption[] = configs.map((c) => ({
        id: asText(c.id),
      ...
      patchState({
        modelOptions: [],
        message:
          safeErrorMessage(e) === "__TIMEOUT__"
            ? t.noModels
            : safeErrorMessage(e),
      });
    } finally {
  ...
  function handleSelectModel(id: string): void {
...
function buildConfigureParams(s: SettingsState, withTest: boolean) {
    ...
    if (s.modelConfigId) params.model_config_id = s.modelConfigId;
    // 输入框为明文密码框，有值即视为用户填写的真实密钥，直接提交。
    // 注意：不再用掩码判断，因为一旦用掩码会挡住真实密钥导致 configured 永远 false。
    if (s.appSecret) params.app_secret = s.appSecret;
    if (s.modelApiKey) params.model_api_key = s.modelApiKey;
    if (withTest) params.test_connection = true;
    return params;
  }

  // 给任意 Promise 工厂包一层超时（用于沙箱 API 直调）
  function callFnWithTimeout<T>(
    fn: () => Promise<T>,
    ms: number
  ): Promise<T> {
  ...
  async function handleAction(action: RuntimeAction): Promise<void> {
    if (busyRef.current) return;
    busyRef.current = true;
    ...
    const flags: Partial<SettingsState> = {};
    if (action === "save_credentials") flags.saving = true;
    if (action === "save_and_test") flags.saving = true;
    if (action === "start_service") flags.starting = true;
    if (action === "stop_service") flags.stopping = true;
    ...
    try {
      ...
      if (action === "start_service") {
        result = await callToolWithTimeout(
          "feishu_bot:feishu_bot_service_start",
          { restart: true },
          30000
        );
      } else if (action === "stop_service") {
        result = await callToolWithTimeout(
          "feishu_bot:feishu_bot_service_stop",
          {},
          30000
        );
      } else {
        result = await callToolWithTimeout(
          "feishu_bot:feishu_bot_configure",
          buildConfigureParams(s, action === "save_and_test"),
          30000
        );
      }
      ...
      if (action === "save_and_test") {
        const conn = result && result.connection
          ? (result.connection as Record<string, unknown>)
          : undefined;
        patchState({
          message: success
  ...
  children.push(
    UI.Row({ modifier: M.fillMaxWidth().padding(4) }, [
      UI.Button({
  ...
  // 下拉选择模型
  const modelMenuChildren: ComposeNode[] = s.modelOptions.map((opt) =>
    UI.Row(
      {
  ...
  children.push(
    UI.Box({ modifier: M.fillMaxWidth().padding(4) }, [
      UI.Button({
        text:
          s.modelOptions.length === 0
            ? t.noModels
  ...
  // 状态卡片（移到按钮下方）
  children.push(
    UI.Card({ modifier: M.fillMaxWidth(), padding: 12 }, [
...
function placeholderSpacer(UI: { Spacer: (props?: any, children?: any) => ComposeNode }): ComposeNode {
  return UI.Spacer({ height: 4 });
}

export default function feishuSettingsScreen(ctx: ComposeDslContext): ComposeNode {
...
function safeErrorMessage(error: unknown): string {
  const common = require("../../shared/feishu_common.js") as {
    safeErrorMessage: (e: unknown) => string;
  };
